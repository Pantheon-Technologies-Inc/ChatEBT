...
// Temporary: toggle token logs via env flag (defaults to off)
...
class AresTokenRefreshService {
  ...
  start() {
    if (this.isRunning) {
      if (TOKEN_LOGS_ENABLED) logger.warn('[aresTokenRefresh] Service already running');
      return;
    }
  ...
  async runRefreshCycle() {
    ...
    if (TOKEN_LOGS_ENABLED) logger.info('[aresTokenRefresh] Starting refresh cycle');
    ...
    try {
      ...
      const tokensNeedingRefresh = await this.findTokensNeedingRefresh(
        fiveMinutesFromNow,
        thirtyDaysAgo,
      );
      ...
      if (tokensNeedingRefresh.length > 0) {
        if (TOKEN_LOGS_ENABLED)
          logger.info(
            '[aresTokenRefresh] Tokens found for refresh:',
      ...
      if (tokensNeedingRefresh.length === 0) {
        if (TOKEN_LOGS_ENABLED) logger.info('[aresTokenRefresh] No tokens need refresh');
        return;
      }
  ...
  async findTokensNeedingRefresh(expiryThreshold, activityThreshold) {
    try {
      ...
      if (allAresTokens.length > 0) {
        if (TOKEN_LOGS_ENABLED)
          console.log(
            `[DEBUG] Sample token - userId: ${allAresTokens[0].userId}, expires: ${allAresTokens[0].expiresAt}, created: ${allAresTokens[0].createdAt}`,
          );
      }
      ...
      for (const refreshToken of refreshTokens) {
        ...
        if (!hasAccessToken) {
          if (TOKEN_LOGS_ENABLED)
            console.log(
              `[DEBUG] User ${refreshToken.userId} has valid refresh token but no access token - needs refresh`,
            );
      ...
      if (allTokensNeedingRefresh.length > 0) {
        if (TOKEN_LOGS_ENABLED)
          console.log(
            `[DEBUG] First token: userId=${allTokensNeedingRefresh[0].userId}, expires=${allTokensNeedingRefresh[0].expiresAt}`,
          );
        if (!allTokensNeedingRefresh[0]._needsRefresh) {
          if (TOKEN_LOGS_ENABLED)
            console.log(
              `[DEBUG] Token expires in ${Math.round((allTokensNeedingRefresh[0].expiresAt - new Date()) / 60000)} minutes`,
            );
        } else {
          if (TOKEN_LOGS_ENABLED)
      ...
      if (TOKEN_LOGS_ENABLED)
        logger.info('[aresTokenRefresh] Database query completed', {
          ...
          tokens: allTokensNeedingRefresh.map((t) => ({
            ...
            minutesUntilExpiry: t._needsRefresh
              ? 'expired'
              : Math.round((t.expiresAt - new Date()) / 60000),
          })),
        });
  ...
  async refreshUserToken(userId, accessToken) {
    try {
      ...
      if (accessToken._needsRefresh) {
        if (TOKEN_LOGS_ENABLED)
          logger.info(
            '[aresTokenRefresh] Access token missing, performing direct refresh using refresh token',
            { userId },
          );
